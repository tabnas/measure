...
with tabnas's terminology, this repository's file layout (a README and
two pages under `docs/`, with no per-runtime doc set), and
executable-example conventions. This guide is normative for every page `scripts/gated-docs.mjs`
...
(`scripts/gated-docs.mjs`) and one banned list
...
The gated set is the reader-facing one: the `README.md` and the two prose
pages under `docs/`. This repository is a measurement harness rather than
a library, so it has no per-runtime Diátaxis set. The benchmark
manifests, the schemas and the recorded results are data, not prose, and
they are out.
...
a clean run over the gated set: 203 alerts across 3 files. Those numbers
...
and `scripts/vale-counts.mjs` on every push and pull request that
...
and `scripts/docs.test.mjs` runs in `npm test`.
...
The pages under `docs/` are not exempt: they are read by somebody
deciding whether to trust a published measurement, and a citation they
cannot follow tells them nothing.
...
it in CI, and `scripts/docs.test.mjs` reads the same file rather than
...
second list to update, and `scripts/docs.test.mjs` checks that every
