const colors = ['#c8f169', '#68d9b3', '#ff9b62', '#78aef8', '#dc83f2', '#f6d365']
...
if (catalog.latestRunId === null) {
  ...
  renderSummary(catalog, latest, history)
  renderLatest(latest)
...
function renderSummary(catalogData, latest, historyData) {
  const values = [
    ['Recorded runs', catalogData.runs.length],
    ['Benchmarks', latest.benchmarks.length],
    ['Implemented ports', latest.ports.length],
    ['Historical points', historyData.series.reduce((sum, series) => sum + series.points.length, 0)],
  ]
...
function renderHistory(historyData) {
  const selector = document.querySelector('#history-case')
  const measurements = [...new Set(historyData.series.map((series) => `${series.benchmarkId}/${series.caseId}`))]
  selector.innerHTML = measurements.map((key) => `<option value="${escapeHTML(key)}">${escapeHTML(key)}</option>`).join('')
  selector.addEventListener('change', () => drawChart(historyData, selector.value))
  drawChart(historyData, measurements[0])
}

function drawChart(historyData, measurement) {
  const svg = document.querySelector('#history-chart')
  const series = historyData.series.filter(
    (candidate) => `${candidate.benchmarkId}/${candidate.caseId}` === measurement,
  )
  ...
  series.forEach((item, seriesIndex) => {
    ...
    if (coordinates.length > 1) {
      nodes.push(`<polyline points="${coordinates.join(' ')}" fill="none" stroke="${color}" stroke-width="3"/>`)
    }
    item.points.forEach((point) => {
      nodes.push(`<circle cx="${x(Date.parse(point.generatedAt))}" cy="${y(point.medianNs)}" r="6" fill="${color}" stroke="#101311" stroke-width="3"><title>${escapeHTML(item.portLabel)} · parser ${escapeHTML(point.parserVersion)} · ${formatDuration(point.medianNs)} · ${new Date(point.generatedAt).toLocaleString()}</title></circle>`)
    })
  })
  ...
  document.querySelector('#history-legend').innerHTML = series
    .map(
      (item, index) =>
        `<span style="--series:${colors[index % colors.length]}">${escapeHTML(item.portLabel)} · ${escapeHTML(item.runtimeVersion)} · env ${escapeHTML(item.environmentFingerprint.slice(0, 8))}</span>`,
    )
    .join('')
}
